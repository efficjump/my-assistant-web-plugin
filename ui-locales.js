(function initializeUiLocales(global) {
  ...
  const ENGLISH = Object.freeze({
    ...
    "빠른 실행 모델": "Fast execution model",
    "비우면 기본 모델 사용": "Leave blank to use the primary model",
    "빠른 모드의 일반 화면 판단·정책·검증에 먼저 사용하고, 시각 작업·교정·불확실한 결과는 기본 모델로 넘깁니다.": "Used first for normal page decisions, policy checks, and verification in fast mode; visual work, repairs, and uncertain results fall back to the primary model.",
    "응답 속도": "Response speed",
    "빠르게": "Fast",
    "균형": "Balanced",
    "정밀하게": "Thorough",
    "빠른 모드는 낮은 추론량과 짧은 응답을 우선하며 안전 검증에 실패하면 자동으로 정밀 경로로 전환합니다.": "Fast mode favors lower reasoning effort and shorter output, then automatically escalates when safety validation fails.",
    "API 처리 우선순위": "API processing priority",
    "기본 처리": "Standard processing",
    "우선 처리": "Priority processing",
    "OpenAI Responses의 우선 처리는 지연을 줄일 수 있지만 추가 비용이 발생할 수 있습니다.": "OpenAI Responses priority processing can reduce latency but may incur additional cost.",
    "성공한 단순 작업의 빠른 경로를 이 브라우저에서 재사용": "Reuse successful simple-task fast paths in this browser",
    "구조화 출력 사용": "Use structured output",
    "AI 응답을 실시간으로 수신": "Stream AI responses",
    ...
    "JSON 저장": "Save JSON",
    "CSV 저장": "Save CSV",
    "수집 결과": "Collected results",
    "저장된 수집 결과 없음": "No saved collected results",
    "내보낼 수집 결과": "Collected results to export",
    "결과 CSV 저장": "Save result CSV",
    "결과 XLSX 저장": "Save result XLSX",
    "자동화 · 테스트 세트": "Automation · test sets",
    "세트 이름": "Set name",
    "현재 작업 묶음의 이름": "Name for the current group of tasks",
    "자동화 세트 저장": "Save automation set",
    "테스트 세트 저장": "Save test set",
    "저장된 자동화 또는 테스트 세트": "Saved automation or test set",
    "선택 세트 실행": "Run selected set",
    "세트 JSON 내보내기": "Export set JSON",
    "세트 가져오기": "Import set",
    "세트 삭제": "Delete set",
    "대화 기록": "Conversation history",
    "대화 CSV 저장": "Save conversation CSV",
    "수집 결과 없음": "No collected results",
    "저장된 세트 선택": "Select a saved set",
    "저장된 세트 없음": "No saved sets",
    "요소 선택": "Pick element",
    "더보기": "More",
    "보조 기능": "Utilities",
    ...
    "문맥 의존 후속 요청으로 해석": "Interpreted as a context-dependent continuation",
    ...
    "같은 열기·접기 컨트롤을 다시 누르는 계획을 차단했습니다. 현재 화면에서 목표에 맞는 다른 컨트롤을 특정하지 못했습니다.": "A plan to activate the same disclosure control again was blocked. No different control matching the goal could be identified on the current page.",
    "직전 시도에서 변화가 없었던 같은 대상을 다시 실행하지 않았습니다. 현재 화면에서 다른 대상이나 접근을 특정하지 못했습니다.": "The same target that produced no change was not executed again. No different target or approach could be identified on the current page.",
    "같은 열기·접기 컨트롤을 다시 눌러 화면 상태만 되돌리는 반복을 차단했습니다. 현재 화면에서 목표에 맞는 다른 컨트롤을 찾아야 합니다.": "Repeated activation of the same disclosure control was blocked because it would only reverse the page state. A different control matching the goal is required.",
    "직전 시도에서 변화가 없었던 같은 대상을 새 근거 없이 다시 실행하는 계획을 차단했습니다. 다른 대상이나 다른 접근이 필요합니다.": "A plan to retry the same unchanged target without new evidence was blocked. A different target or approach is required.",
    "실질적 진행 없이 같은 표시 컨트롤이 반복됨": "The same disclosure control repeated without material progress",
    "새 화면 근거 없이 실패·무변화 시도를 반복함": "A failed or unchanged attempt repeated without new page evidence",
  ...
  const ENGLISH_PATTERNS = Object.freeze([
    ...
    [/^AI 응답 수신 중 · 응답 연결됨$/u, () => "Receiving AI response · connected"],
    [/^AI 응답 수신 중 · ([0-9,]+)자 수신$/u, ([, chars]) => `Receiving AI response · ${chars} characters`],
