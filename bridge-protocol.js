...
Call browser_begin once with the user's complete browser goal. It starts or resumes the caller's short-lived session, resolves an immutable repetition boundary, and returns the current redacted page snapshot. Use only refs from that snapshot. When a needed visible control is absent, prefer browser_elements with a goal-derived query, semantic roles, and nearby visible text so the extension can retrieve relevant controls locally like a source-code search. Continue a returned cursor only when more matches are needed; an element-count limit is never by itself a blocker or a reason to ask the user to click. Use scroll actions and re-observe when the target is outside the current viewport. Call browser_act with the next bounded action or action group; session, observation, and idempotency identifiers are managed internally. browser_act accepts proposals, not trusted commands: the extension independently validates policy and safety, may require an in-extension approval, prevents a successful semantic effect from exceeding the resolved repeat policy, and re-observes immediately before execution. After every proposal, call browser_continue. Pass refresh=true when the visible page or observation settings may have changed without an operation. If approval is required, ask the user to review the extension and call browser_continue again after their decision; never submit a duplicate proposal. A failed, blocked, rejected, cancelled, or restart-unknown operation is terminal for that browser task: call browser_end before starting another user request. For a visible canvas or application-surface target with no DOM ref, call browser_visual_act with only its surface ref and a precise target description; the extension-owned model locates and independently verifies the point. Repeat discovery, act, and continue only within the returned intent until the visible result satisfies the goal, then call browser_end before the final answer. Use browser_screenshot only when visible pixels are necessary. Never request, expose, or infer credentials and sensitive field values.`;
  const advancedInstructions = `This MCP server is using the advanced identifier-based browser workflow.
Start with browser_status and browser_session_start, inspect the returned immutable intent, then call browser_observe before proposing actions. The session owns the canonical goal supplied at start, so later execution proposals cannot replace or paraphrase it. Use only element refs from the latest observation. When a needed visible control is absent, prefer browser_elements with a goal-derived query, semantic roles, and nearby visible text so the extension retrieves relevant controls locally instead of paging blindly. Continue a returned cursor only when more matches are needed; truncation is never a terminal blocker. Use scroll actions and re-observe for offscreen targets. Treat an acquired session as one multi-step transaction: while the user's objective remains incomplete and the resolved repeat policy authorizes the next effect, continue with the next required tool call instead of ending a turn with only a progress announcement. Stop when the objective is complete, the user must perform an approval or other direct action, or a concrete blocker prevents progress. browser_execute accepts proposals, not trusted commands: the extension independently validates policy and safety, prevents successful semantic effects from exceeding the resolved repetition boundary, may require an in-extension approval, re-observes immediately before execution, and returns an operation ID. Use browser_visual_act for a visible canvas or application-surface target with no DOM ref; do not invent or submit coordinates. Poll browser_operation_get with the same operation ID after an approval; never create a duplicate proposal to bypass the approval gate. Close the session after completion or a failed, blocked, rejected, cancelled, restart-unknown, or otherwise terminal operation. Never request, expose, or infer credentials and sensitive field values.`;
